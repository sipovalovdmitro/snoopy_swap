...
describe("SnoopySwap", function () {
  ...
  async function deploySnoopySwapFixture() {
    const faucet = await ethers.getImpersonatedSigner(
    ...
    await faucet.sendTransaction(tx);
    const SnoopySwap = await ethers.getContractFactory("SnoopySwap");
    const snoopySwap = await SnoopySwap.connect(faucet).deploy();
    ...
    const snoopy = new ethers.Contract(snoopyAddr, snoopyABI, snoopyowner);
    ...
    return { snoopySwap, snoopy, snoopyowner, uniswapV2Router };
  }

  describe("Deployment", function () {
    it("Should be successful to swap the snoopy tokens", async function () {
      const { snoopySwap, snoopy, snoopyowner, uniswapV2Router } =
        await loadFixture(deploySnoopySwapFixture);

      const snoopyBalance = await snoopy.balanceOf(snoopyowner.address);
      console.log(
        `Snoopy Balance of the owner: ${ethers.utils.formatUnits(
          snoopyBalance,
          6
        )} SNOOPY`
      );
      await expect(
        snoopy
          .connect(snoopyowner)
          .approve(uniswapV2Router.address, snoopyBalance)
      ).to.not.be.reverted;
      ...
      await expect(
        uniswapV2Router.swapExactTokensForETH(
          ...
          0,
          [
            "0x6ce3E45e73aE1Effa3CC8aE2F1620F4A18258391",
