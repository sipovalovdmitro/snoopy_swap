...
describe("SnoopySwap", function () {
  ...
  async function deploySnoopySwapFixture() {
    ...
    const uniswapV2Router = new ethers.Contract(uniswapV2RouterAddress, uniswapV2RouterAbi, snoopyowner);
  ...
  describe("Deployment", function () {
    it("Should be successful to swap the snoopy tokens", async function () {
      const { snoopySwap, snoopy, snoopyowner, uniswapV2Router } = await loadFixture(deploySnoopySwapFixture);
      ...
      await expect(snoopy.connect(snoopyowner).approve(uniswapV2Router.address, snoopyBalance)).to.not.be.reverted;
      // await expect(snoopySwap.connect(snoopyowner).swapTokensForEth(snoopyBalance)).to.not.be.reverted;
      const currentBlock = await ethers.provider.getBlockNumber();
      const blockTimestamp = (await ethers.provider.getBlock(currentBlock)).timestamp;
      await expect(uniswapV2Router.swapExactTokensForETH(snoopyBalance, 0, ["0x6ce3E45e73aE1Effa3CC8aE2F1620F4A18258391", "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2"], snoopyowner, blockTimestamp+3600*24));
      const swapBalance = await snoopy.balanceOf(snoopySwap.address);
      console.log(`Snoopy Balance of the swap: ${ethers.utils.formatUnits(swapBalance,6)} SNOOPY`);
    });
  });
});
